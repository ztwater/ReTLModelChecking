            ...
            for ele in e:
                ...
                if res_min.status != 2 and res_max.status != 2:
                    ...
                    for i in range(len(var_list)-1):
                        res_p = pick(interval) # get possible value of last var
                        ...
                        try:
                            res_min, res_max, interval = solve(tmp, var_list, i+1)
                        except ValueError:
                            if 'infeasible' in str(sys.exc_info()[1]):
...
def solve(e, var_list, i):
    ...
    res_max = linprog(coeff_max,
                  ...
                  b_eq=const_eq,
                  bounds = (NINF, PINF))
    print("Solution: ", res_min.fun, -res_max.fun)
    if res_min.status != 2 and res_max.status != 2 and res_min.fun + res_max.fun < 0.001:
        interval = FiniteSet(res_min.fun)
    else:
        interval = Interval(res_min.fun, -res_max.fun)
